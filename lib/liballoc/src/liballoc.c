...
void *PREFIX(malloc)(size_t req_size)
{
	...
	while ( maj != NULL )
	{
		...
		if ( maj->first == NULL )
		{
			maj->first = (struct liballoc_minor*)((u32)maj + sizeof(struct liballoc_major) );
		...
		if ( diff >= (size + sizeof(struct liballoc_minor)) )
		{
			...
			maj->first->prev = (struct liballoc_minor*)((u32)maj + sizeof(struct liballoc_major) );
		...
		while ( min != NULL )
		{
				...
				if ( min->next == NULL )
				{
					...
					{
						// yay....
						min->next = (struct liballoc_minor*)((u32)min + sizeof( struct liballoc_minor ) + min->size);
						min->next->prev = min;
						min = min->next;
						min->next = NULL;
				...
				if ( min->next != NULL )
				{
					...
					{
						// yay......
						new_min = (struct liballoc_minor*)((u32)min + sizeof( struct liballoc_minor ) + min->size);
