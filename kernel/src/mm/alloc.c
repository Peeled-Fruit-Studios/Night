...
void *ksbrk(int n) {
  ...
  if ((kern_heap + (n * PAGESIZE)) > (char *)KERN_HEAP_LIM) {
    puts("PANIC: ksbrk(): no virtual memory left for kernel heap !\n");
    return (char *)-1;
  }
  ...
  for (i = 0; i < n; i++) {
    ...
    if ((int)(p_addr) < 0) {
      puts("PANIC: ksbrk(): no free page frame available !\n");
      return (char *)-1;
    }
...
void *kmalloc(unsigned long size) {
  ...
  while (chunk->used || chunk->size < realsize) {
    if (chunk->size == 0) {
      puts(
          "\nPANIC: kmalloc(): corrupted chunk on  with null size (heap %x) "
          "!\nSystem halted\n",
          chunk, kern_heap);
      // error
      asm("hlt");
...
void kfree(void *v_addr) {
  if (v_addr == (void *)0) return;

  struct kmalloc_header *chunk, *other;

  /* On libere le bloc alloue */
  chunk =
      (struct kmalloc_header *)((u32)v_addr - sizeof(struct kmalloc_header));
  chunk->used = 0;
