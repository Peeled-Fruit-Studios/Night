...
page *get_page_from_heap(void) {
  ...
  if ((int)(p_addr) < 0) {
    puts(
        "PANIC: get_page_from_heap(): no page frame available. System halted "
        "!\n");
  }
  ...
  if (list_empty(&kern_free_vm)) {
    puts(
        "PANIC: get_page_from_heap(): not memory left in page heap. System "
        "halted !\n");
  }
...
int release_page_from_heap(char *v_addr) {
  ...
  if (p_addr) {
    ...
    puts(
        "WARNING: release_page_from_heap(): no page frame associated with "
        "v_addr %x\n",
        v_addr);
    return 1;
  }
  ...
  list_for_each_entry(next_area, &kern_free_vm, list) {
    if (next_area->vm_start > v_addr) break;
  }
  ...
  if (prev_area->vm_end == v_addr) {
    ...
    puts(
        "\nPANIC: release_page_from_heap(): corrupted linked list. System "
        "halted !\n");
    asm("hlt");
  }
...
void init_memory(u32 high_mem) {
  ...
  for (pg = 0; pg < pg_limit / 8; pg++) mem_bitmap[pg] = 0;

  for (pg = pg_limit / 8; pg < RAM_MAXPAGE / 8; pg++) mem_bitmap[pg] = 0xFF;
...
struct page_directory *pd_create(void) {
  ...
  for (i = 0; i < 256; i++) pdir[i] = pd0[i];

  /* Espace utilisateur */
  for (i = 256; i < 1023; i++) pdir[i] = 0;
...
*/
/*
 * Cree et initialise un rep. de pages pour une tache en copie d'une autre  (ne
 * marche pas encore )
 */
...
int pd0_add_page(char *v_addr, char *p_addr, int flags) {
  ...
  if (v_addr > (char *)USER_OFFSET) {
    puts("ERROR: pd0_add_page(): %p is not in kernel space !\n", v_addr);
    return 0;
  }
 ...
 */
int pd_add_page(char *v_addr, char *p_addr, int flags,
                struct page_directory *pd) {
   ...
   *   PageTable. Les 12 derniers bits permettent de modifier une entree du
   * PageTable
